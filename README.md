...
На днях Илье пришли его новенькие наручные часы фирмы MI. Те самые,
...
сходить за списком в API и вывести твиты на консоль. Всё остальное - дело техники!

Конечно, можно получить настоящие сообщения, обратившись по урлу
`https://api.twitter.com/1.1/search/tweets.json?q=%23urfu-testing-2016`. Но для этого надо
заморочиться с авторизацией и изучить мегабайты документации. Чтобы поиграть
с девайсом можно подменить настоящий запрос при помощи библиотеки [nock](https://github.com/node-nock/nock).
В ответе вернется список твитов, из которого нужно взять только время публикации
`created_at` и содержание твита `text`.
```json
[
    {
...
Текст твита выводится на консоль как есть, а вот дату Илья хочет видеть в следующем виде:
  ...
  4. Если сообщение датируется прошлым годом, то это нужно обязательно упомянуть: `"2015-03-25T15:09:10.609Z" -> "25 марта 2015 года в 15:09"`
