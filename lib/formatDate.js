module.exports = function formatDate(date) {
    ...
    else if (new Date().getFullYear() - Number(year)<0)//Проверка года
        throw new TypeError('Twit from the future Year = ' + year);   
    else if( new Date().getDate() - day < 0 && new Date().getMonth()+1 - month == 0)//Проверка дня
        throw new TypeError('Twit from the future Day = ' + day); 
    else if( new Date().getFullYear() - Number(year) == 0 && new Date().getMonth()+1 - month < 0)//Проверка месяца
        throw new TypeError('Twit from the future Month = ' + month); 

    if(new Date().getDate() - day == 1 )
        dateReturn = "вчера в ";
    else if((new Date().getMonth()+1 - month > 0 || new Date().getDate() - day > 1) && new Date().getFullYear() - Number(year)==0 )
    {
